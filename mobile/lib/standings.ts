// Ported 1:1 from src/lib/standings.ts on the web side so mobile and web
// produce identical standings for the same inputs.
...
export const FALLBACK_NETBALL_SCORING: ScoringSystem = {
  id: 'default',
...
export function calculateStandings(
  ...
  const sys: ScoringSystem = scoringSystem || FALLBACK_NETBALL_SCORING;
      ...
      if (rule.startsWith('head_to_head')) {
        ...
        if (h2hMatches.length > 0) {
          ...
          for (const m of h2hMatches) {
            ...
            const mp = pointsForMatch(aScore, bScore, sys);
            aPts += mp.home;
            bPts += mp.away;
            aGd += aScore - bScore;
            bGd += bScore - aScore;
