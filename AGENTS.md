...
**Working from the roadmap (clarify → plan → stop → log progress).** If no item is named, `pull` (or `list`) and ask which to take; if one is named, `show` it (disambiguate if a title substring matches several) **and read the full GitHub issue body + comments**. Read its Acceptance criteria, then **ask clarifying questions until you understand what "done" means** (scope, edge cases, affected code, data/scoring/RLS, 375px/1280px, test impact). Then set it `In progress` (and write back any sharpened `--acceptance`), produce an implementation plan, and **stop for the user's go-ahead before writing code**. As you work, **keep the item updated** with `node scripts/roadmap.mjs log "<item>" --body "…"` (or `set --note`): post the plan at the start, a note at each milestone, and a **detailed completion summary — never mark `Completed` without recording what was done**. If blocked set `Blocked` with a `--deps` note and a comment explaining why.
...
**Pilot scope.** The first go-to-market pilot is a concierge-supported UK netball competition. What's in and out of scope for that pilot (and the organiser-facing one-pager) is documented in [`docs/pilot/`](docs/pilot/uk-netball-pilot-scope.md) — read it before promising pilot behaviour.
